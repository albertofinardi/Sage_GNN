...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{Memory explosion with real fanout configuration [15, 10, 10, 10, 10]. Each layer multiplies the sampled nodes, reaching 150,000 neighbors per seed node at layer 5. Combined across 128 seed nodes per batch, this yields 19.2M nodes consuming 30--40 GB of GPU memory.}
...
\paragraph{Neighbor Sampling (16 configurations).} We explored diverse fanout strategies to balance accuracy, training time, and memory consumption:
\begin{itemize}
    \item \textbf{Uniform:} [10,10,10,10,10], [12,12,12,12,12],\newline[15,15,15,15,15]
    \item \textbf{Decreasing:} [20,15,10,5,3], [15,15,10,5,5],[30,10,5,3,3], \newline[12,11,10,9,8]
    \item \textbf{Increasing:} [3,5,10,15,20]
    \item \textbf{Layer ablation:} [50,0,0,0,0], [15,0,0,0,0], [15,10,0,0,0], \newline[15,10,10,0,0], [15,10,10,10,0]
\end{itemize}
The total fanout metric, $F_{\text{total}} = \prod_{i=1}^{5} S_i$, quantifies the exponential neighborhood expansion per seed node.
...
Each configuration was trained for up to 150 epochs with early stopping (patience 10) based on validation accuracy. Training time ranged from 7.22 hours ([15,10,10,10,0]) to over 19.53 hours \newline([15,10,10,10,10]) depending on configuration.
...
All benchmarking experiments were conducted on the MeluXina supercomputer (Luxembourg National Supercomputer) using the Accelerator Module with 4x NVIDIA A100 GPUs (40GB each) per node. We evaluate three key dimensions: mini-batch sampling, neighbor sampling strategies, and GPU scaling.
...
\begin{figure}[H]
    ...
    \caption{Peak memory consumption scales linearly with batch size. Maximum feasible batch size is 160, using ${\sim}32$ GB (with ${\sim}7.5$ GB system overhead).}
...
Neighbor sampling strategy critically impacts both accuracy and computational cost. We tested 16 fanout configurations, including uniform strategies, decreasing patterns, and layer ablation studies. The fanout determines the receptive field size and thus the amount of structural context available to the model, but also directly affects memory consumption through exponential neighborhood expansion. Figures~\ref{fig:neighbor-acc-time} and \ref{fig:neighbor-memory} illustrate the trade-offs between accuracy, training time, and memory consumption across various fanout configurations.
...
\textbf{Key Findings:}
\begin{itemize}
    \item \textbf{Optimal efficiency:} Configuration [15,10,10,10,0] achieves near-baseline accuracy (81.14\% vs. 80.96\%) in only 7.22 hours, representing a \textbf{2.7$\times$ speedup} compared to the baseline [15,10,10,10,10]. This suggests that \textbf{the final layer can use zero fanout} without significant accuracy loss, as 4-hop aggregation captures sufficient structural context.
...
\begin{figure}[htbp]
    ...
    \caption{Training loss increases with GPU count due to larger effective batch sizes (512 for 4 GPUs, 1024 for 8 GPUs), resulting in wider minima and reduced gradient noise.}
...
\begin{itemize}
    ...
    \item \textbf{Loss-speed trade-off:} Larger effective batch sizes (batch\_size $\times$ world\_size = 512 for 4 GPUs, 1024 for 8 GPUs) lead to higher training loss due to reduced gradient noise and wider minima. However, test accuracy remains competitive, justifying the 7$\times$ wall-clock speedup for distributed training.
\end{itemize}
...
\paragraph{Loss Convergence Trade-off.} Larger effective batch sizes (product of batch\_size, accum\_steps, and world\_size) lead to higher training loss. Effective batch sizes of 512 (4 GPUs) and 1024 (8 GPUs) reduce gradient noise, resulting in convergence to wider minima with higher loss. However, test accuracy remains competitive, indicating that the loss increase does not significantly harm generalization - an acceptable trade-off for substantial wall-clock speedup.

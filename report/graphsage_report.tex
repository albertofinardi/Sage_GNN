...
% Document metadata
\title{\textbf{GraphSAGE: Inductive Representation Learning on Large Graphs}\\[0.5em]
...
\begin{abstract}
This report presents an in-depth analysis of GraphSAGE (Graph SAmple and aggreGatE)~\cite{graphsage}, an inductive framework for learning node embeddings on large-scale graphs. Unlike transductive approaches that require all nodes to be present during training, GraphSAGE learns a function that generates embeddings by sampling and aggregating features from a node's local neighborhood. We discuss the theoretical foundations of graph neural networks, the key innovations of GraphSAGE, our implementation details, hyperparameter choices, and experimental results on the ogbn-products benchmark dataset.
\end{abstract}
...
Graph-structured data is ubiquitous in real-world applications, from social networks and citation graphs to biological protein interaction networks and recommendation systems. A fundamental challenge in machine learning on graphs is learning meaningful representations (embeddings) of nodes that capture both their features and their structural role in the graph.
...
\begin{itemize}
    \item \textbf{DeepWalk / node2vec:} These methods~\cite{deepwalk,node2vec} learn embeddings via random walks on the graph. However, they are \textit{transductive}—they learn a fixed embedding for each node in the training graph. Adding new nodes requires retraining on the entire graph. \textit{Note: These are not GNNs; they do not use message passing.}
...
\begin{quote}
\textit{Don't learn embeddings for each node—learn a \textbf{function} that generates embeddings.}
\end{quote}
